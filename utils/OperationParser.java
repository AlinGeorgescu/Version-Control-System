...
final class OperationParser {
    ...
    AbstractOperation parseOperation(final String newOperation) {
        ...
        if (operationArgs.get(0).toLowerCase().equals("vcs")) {
            return parseVcsOperation(operationArgs);
        } else {
            return parseFileSystemOperation(newOperation, operationArgs);
        }
    }

    private AbstractOperation parseVcsOperation(final ArrayList<String> operationArgs) {
    ...
    private AbstractOperation parseFileSystemOperation(final String newOperation,
    ...
    private ArrayList<String> parsePathAndName(final String toParse) {
